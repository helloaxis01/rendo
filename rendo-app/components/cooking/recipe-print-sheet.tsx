...
function RecipePrintLayout({
  ...
  return (
    <article className={className} aria-hidden={hidden || undefined}>
      ...
      <div className={gridClassName ?? "recipe-print-grid"}>
        <section className="recipe-print-column recipe-print-column-ingredients">
          <h2 className="recipe-print-section-title">
            {RECIPE_PRINT_LABELS.ingredients}
          </h2>
              ...
              <ul className="recipe-print-ingredients">
                {group.items.map((item, itemIndex) => (
                  ...
                  >
                    ...
                    <span className="recipe-print-ingredient-text">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <section className="recipe-print-column recipe-print-column-steps">
          <h2 className="recipe-print-section-title">
            {RECIPE_PRINT_LABELS.directions}
          </h2>
          {content.steps.length > 0 ? (
            ...
            <p className="recipe-print-step-body">No directions listed.</p>
          )}
        </section>
      </div>
...
export function RecipePrintSheet({ recipe, servings, unitSystem }: Props) {
  ...
  return createPortal(
    <RecipePrintLayout content={content} className="recipe-print-sheet" hidden />,
    document.body
  );
}

export function RecipePrintPreview({ recipe, servings, unitSystem }: Props) {
  const content = buildRecipePrintContent(recipe, servings, unitSystem);
  ...
  return (
    <RecipePrintLayout
      ...
      gridClassName="recipe-print-grid recipe-print-grid-preview"
    />
  );
}
